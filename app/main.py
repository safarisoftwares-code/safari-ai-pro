...
<script>function togglePw(){var i=document.getElementById('pwInput');if(i.type==='password'){i.type='text';}else{i.type='password';}}</script>
    ...
    for k in api_keys:
        ...
        api_rows += f"<tr><td>{email}</td><td>{k.plan}</td><td>0/{k.daily_limit}</td><td>0</td><td><code>{k.key[:16]}...</code></td><td><form method='post' action='/admin/revoke-key' style='display:inline'><input type='hidden' name='key' value='{k.key}'><input type='hidden' name='pw' value='{pw}'><button type='submit' style='background:#d32f2f;color:#fff;border:none;padding:5px 10px;border-radius:5px;cursor:pointer'>Revoke</button></form></td></tr>"
...
.c{{max-width:1200px;margin:auto;background:#fff;border-radius:20px;box-shadow:0 10px 40px rgba(0,0,0,.15);padding:30px}}
