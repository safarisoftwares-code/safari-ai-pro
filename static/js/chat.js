...
window.clearAllChats=async function(){
    ...
    if(token){
        ...
        for(var i = 0; i < ids.length; i++){
            try { await fetch('/api/v1/chat/delete-chat?session_id=' + ids[i] + '&token=' + encodeURIComponent(token), {method:'DELETE'}); } catch(e) {}
        }
    }
...
async function deleteChat(id){
    var token = localStorage.getItem('safari_token');
    if(token){
        try { await fetch('/api/v1/chat/delete-chat?session_id=' + id + '&token=' + encodeURIComponent(token), {method:'DELETE'}); } catch(e) {}
    }
...
var fullQuestion = question ? question + '\n\n[File content from ' + pendingFile.name + ']:\n' + fileContent : 'Please analyze this file: ' + pendingFile.name + '\n\n[File content]:\n' + fileContent;
askForm.append('question',fullQuestion);var token=localStorage.getItem('safari_token');if(token)askForm.append('token',token);var askResp=await fetch('/api/v1/chat/ask',{method:'POST',body:askForm});var askData=await askResp.json();var errMsg2=askData.response||askData.detail||JSON.stringify(askData);chats[activeChat].messages.push('S:'+errMsg2);chats[activeChat].timestamps.push(Date.now());}else{chats[activeChat].messages.push('S:'+(uploadData.detail||'Upload failed.'));chats[activeChat].timestamps.push(Date.now());}pendingFile=null;}else{var form=new FormData();form.append('question',question);form.append('session_id',activeChat);var token=localStorage.getItem('safari_token');if(token)form.append('token',token);var r=await fetch('/api/v1/chat/ask',{method:'POST',body:form});var d=await r.json();var errMsg3=d.response||d.detail||JSON.stringify(d);chats[activeChat].messages.push('S:'+errMsg3);chats[activeChat].timestamps.push(Date.now());}}catch(e){chats[activeChat].messages.push('S:Connection error.');chats[activeChat].timestamps.push(Date.now());}saveChats();renderMessages();setProcessing(false);}
function selectFile(input){var file=input.files[0];if(!file)return;if(file.size>10*1024*1024){showToast('File too large. Maximum 10MB.');input.value='';return;}pendingFile=file;pendingImage=null;showFilePreview(file);var preview=document.getElementById('filePreview');var displayName = file.name.length > 8 ? file.name.substring(0, 8) + '.' : file.name;
document.getElementById('fileName').textContent=displayName;
document.getElementById('fileName').title=file.name;preview.style.display='flex';var imgPreview=document.getElementById('imagePreview');if(imgPreview)imgPreview.style.display='none';showToast('File ready. Type question and Ask.');}
function selectImage(input){var file=input.files[0];if(!file)return;if(file.size>10*1024*1024){showToast('Image too large. Maximum 10MB.');input.value='';return;}pendingImage=file;pendingFile=null;var displayName = file.name.length > 8 ? file.name.substring(0, 8) + '.' : file.name;
var preview=document.getElementById('filePreview');
if(preview){document.getElementById('fileName').textContent=displayName;
    ...
    try {
        ...
        if(d.status === 'success'){
            ...
            d.chats.forEach(function(c){
                serverChats[c.session_id] = {name: c.name, messages: c.messages || [], timestamps: (c.messages || []).map(function(){ return Date.now(); })};
            });
