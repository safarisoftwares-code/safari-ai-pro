...
function selectFile(input){var file=input.files[0];if(!file)return;if(file.size>10*1024*1024){showToast('File too large. Maximum 10MB.');input.value='';return;}pendingFile=file;pendingImage=null;var preview=document.getElementById('filePreview');var displayName = file.name.length > 8 ? file.name.substring(0, 8) + '.' : file.name;
